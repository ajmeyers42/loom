# Engagement root: `LOOM_ENGAGEMENTS_ROOT`
...
| Variable | Meaning |
|----------|---------|
| `LOOM_ENGAGEMENTS_ROOT` | Absolute path to a directory that **contains** one folder per engagement |
...
`"${LOOM_ENGAGEMENTS_ROOT:-$HOME/engagements}/{slug}/"`

Override only if you want engagements somewhere else:

```bash
export LOOM_ENGAGEMENTS_ROOT="/custom/path"
```

Engagement workspace for slug `{slug}`:

`$LOOM_ENGAGEMENTS_ROOT/{slug}/`
...
3. Point the agent at **`$LOOM_ENGAGEMENTS_ROOT/{slug}`** when running the orchestrator or a single skill.

## Agents

Skills and [`AGENTS.md`](../AGENTS.md): if `LOOM_ENGAGEMENTS_ROOT` is unset, use **`$HOME/engagements`** before asking the SA.

## Cloud-synced root (Google Drive / OneDrive)

You can point `LOOM_ENGAGEMENTS_ROOT` at a cloud-synced folder for cross-device access. Engagement files are written locally first; the sync client uploads in the background.
...
```bash
# Add to ~/.zshrc
export LOOM_ENGAGEMENTS_ROOT="$HOME/Library/CloudStorage/GoogleDrive-you@elastic.co/My Drive/engagements"

# Create the folder
mkdir -p "$LOOM_ENGAGEMENTS_ROOT"
```
...
export LOOM_ENGAGEMENTS_ROOT="$HOME/Library/CloudStorage/OneDrive-Elastic/engagements"
mkdir -p "$LOOM_ENGAGEMENTS_ROOT"
```
...
| Topic | Detail |
|-------|--------|
| **Spaces in path** | `My Drive` contains a space. Always quote the env var in shell scripts: `"${LOOM_ENGAGEMENTS_ROOT}"`. The pipeline skills already quote it correctly. |
...
| **Migrating an existing root** | Copy with `cp -Rp ~/engagements/. "$LOOM_ENGAGEMENTS_ROOT/"`, verify in Drive, then `rm -rf ~/engagements`. |
