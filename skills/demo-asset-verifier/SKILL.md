...
description: >
  ...
  ready for demo-bootstrap-generator. No custom API code is written here — skills author
  ...
  This skill MUST complete before demo-bootstrap-generator runs — bootstrap-generator
...
If `.env` is missing: stop and tell the SA to run `demo-cloud-provision` first.
...
`demo-bootstrap-generator` will not run without it.
...
```
Record actual field names. These override any field names from prior demobuilder sessions.
...
This is a no-go blocker for `demo-bootstrap-generator`.
...
─────────────────
  ...
  Failed: {N}  ← BLOCKED (resolve before running demo-bootstrap-generator)
...
NEXT STEP
─────────
  Review asset-bundle/ contents, then run: demo-bootstrap-generator
  (demo-bootstrap-generator reads asset-bundle/asset-schema.json and
...
`demo-bootstrap-generator` can run immediately.
...
re-run `demo-data-modeler` for that index's seed spec, re-run `demo-asset-verifier`. No deployment
