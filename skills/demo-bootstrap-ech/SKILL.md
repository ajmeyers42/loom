---
name: demo-bootstrap-ech
description: >
  ECH (Elastic Cloud Hosted) deployment variant for demo-bootstrap-generator. Generates
...
```
skills/demo-deploy/references/terraform-patterns.md    ← ECH HCL patterns
skills/demo-deploy/references/feature-compatibility.md ← version-specific behavior
skills/demo-deploy/references/inference-config.md      ← ELSER/EIS config for ECH
skills/demo-deploy/references/kibana-api-registry.md   ← API shapes (used in bootstrap-data.py)
skills/demo-deploy/references/pipeline-constants.md    ← thresholds, UUID5 namespace, header values
...
Read `skills/demo-deploy/templates/bootstrap-template.py` as the base structure.
...
def demobuilder_tags() -> list: return [f"demobuilder:{_engagement_id_for_tag()}"]
def merge_tags(existing): return sorted(set((existing or []) + demobuilder_tags()))
