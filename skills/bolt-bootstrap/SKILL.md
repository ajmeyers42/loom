...
description: >
  ...
  finish-verify and generates the deployment artifacts for the target deployment
  ...
  deployment scripts", "generate main.tf", or when finish-verify has completed
  and the SA wants to proceed to deployment artifacts. Requires asset-bundle/
  from finish-verify — will not run without it (D-045).
---
...
`finish-verify`'s job. You translate the `asset-bundle/asset-index.json` and
...
### Required files
```
{engagement_dir}/deploy/asset-bundle/asset-schema.json   ← written by finish-verify
{engagement_dir}/deploy/asset-bundle/asset-index.json    ← written by finish-verify
...
⛔ asset-bundle/ not found or incomplete.
   Run finish-verify first. bolt-bootstrap requires verified
...
| Value | Skill to call |
|---|---|
| `ech` | Read `../bolt-ech/SKILL.md` |
| `serverless` | Read `../bolt-serverless/SKILL.md` |
| `eck` | Read `../bolt-eck/SKILL.md` |
...
OPERATIONS = [
    ...
    "step7_manifest_write",      # D-039: write/update loom-manifests/{engagement_id}
]
```
...
| 7 | `loom_tags()` and `merge_tags()` defined in `bootstrap-data.py` (D-026) | Include in template credential block |
| 8 | All Terraform resources have `tags` or labels from `loom_tags()` where the resource type supports it (D-026) | Add tag block to each resource |
